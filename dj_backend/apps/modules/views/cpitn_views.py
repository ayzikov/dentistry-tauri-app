...
class CPITNListCreateView(APIView):
    def get(self, request: Request, patient_id: int):
        """
        Получение списка
        """
        list_indexes = crud.indexes_get_list(patient_id, index_name="cpitn")
    ...
    def post(self, request: Request, patient_id: int):
        ...
        crud.cpitn_create(teeth, patient_id)

        # делаем редирект на получение списка индексов CPITN
        redirect_url = reverse("modules:cpitn:list_create", args=[patient_id])
